...
pub async fn search_author_book(
    name: &String,
    pool: &MySqlPool,
) -> sqlx::Result<Vec<BookSearchAuthor>> {
...
pub async fn list_books_sort_asc(
    start: i32,
    length: i32,
    pool: &MySqlPool,
) -> AppResult<Vec<Book>> {
...
pub async fn take_price(books_id: &Vec<String>, pool: &MySqlPool) -> sqlx::Result<Vec<BookPrice>> {
    let mut query: QueryBuilder<'_, MySql> =
        QueryBuilder::new("select id book_id, price price_each from book where id in (");
